defmodule Djbot.ActiveStates do
  ...
  def set_active(guild_id, active?), do: GenServer.call(__MODULE__, {:set_state, guild_id, active?})

  def set_playing(guild_id, playing), do: GenServer.call(__MODULE__, {:set_playing, guild_id, playing})

  def handle_call({:get, guild_id}, _from, state), do: {:reply, Map.get(state, guild_id, {false, nil}), state}
